...
class MotorControlNode:
    def __init__(self):
        ...
        self.speed = 0.0  # 현재 속도 (초기값: 정지 상태)
    ...
    def control_callback(self, msg):
        ...
        if key == 'w':  # 속도 증가 (전진)
            if self.speed < 0:
                self.speed = 0  # 후진에서 전진으로 변경할 때 0으로 리셋
            self.speed = min(1.0, self.speed + 0.1)  # 최대 1.0
            rospy.loginfo(f"Motor forward (speed: {self.speed})")
            self.motor_hat.set_throttle(self.speed)

        elif key == 's':  # 속도 감소 (후진)
            if self.speed > 0:
                self.speed = 0  # 전진에서 후진으로 변경할 때 0으로 리셋
            self.speed = max(-1.0, self.speed - 0.1)  # 최소 -1.0
